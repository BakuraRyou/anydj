...
async function app(t,analysis,extra={}) {
  ...
  return {base,post(body=Buffer.alloc(STRUCTURE_RATE*5),headers={},signal){return fetch(base+'/api/analysis/structure',{method:'POST',headers:{'Content-Type':'application/octet-stream','X-AnyDj-Local':'1',...headers},body,signal});}};
}
test('Songstruktur-API cached validierte Ergebnisse und entfernt temporäres Audio',async t=>{
  ...
  assert.equal((await client.post(undefined,{'X-AnyDj-Local':''})).status,403);
