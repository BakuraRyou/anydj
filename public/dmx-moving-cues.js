...
export function movingCues(plan,mode,mood='balanced'){
  ...
  let phraseIndex=0,sectionIndex=0,ordinal=0;
  for(let index=0;index<arrangement.times.length;index++){
    ...
    const kind=mode==='wash'?'wash':mood==='atmospheric'?'sweep':event?.kind||phrase?.kind||'bounce';
    const calm=kind==='wash'||['held','quiet','break','outro'].includes(section?.look);
    ...
    if(gap<Math.max(profile.spacing,calm?1.5:kind==='sweep'?.75:.38))continue;
    ...
    const direction=(event?.alternate??ordinal%2)?-1:1;
    const pose=Array.from({length:4},(_,i)=>{
    ...
    const required=1.5*distance(previous.pose,pose)/profile.speed,fraction=required>gap?gap/required:1;
    ...
    cues.push({time,travel:Math.min(gap,Math.max(profile.travel,calm?.8:.2,1.5*distance(previous.pose,reachable)/profile.speed)),pose:reachable});
    ordinal++;
  }
  return cues;
}
