...
export function createColorPicker(name,onChange){
 const element=document.createElement('details');element.className='dj-color-picker';
 element.innerHTML=`<summary>Farbmodus: <span>Songanalyse</span></summary><div class="dj-color-menu">
 ...
 function render(){
  ...
  const label=track?.colorMode?.name||'Songanalyse';if(q('summary span').textContent!==label)q('summary span').textContent=label;
  q('.color-prerequisite').hidden=Boolean(track);
  q('summary').title=`Farbmodus: ${track?.colorMode?.name||'Songanalyse'}`;q('summary').removeAttribute('aria-disabled');
  q('.color-add').disabled=busy||!track;
 }
