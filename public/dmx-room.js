import {roomStyleId,roomStyleOptions,environmentBrightness} from './dmx-room-style.js';
...
export function roomSettings(value={},layout={width:8,depth:6}){
  ...
  return {environmentBrightness:environmentBrightness(value?.environmentBrightness),style:roomStyleId(value?.style),enabled:value?.enabled===true,width:number(value?.width,layout.width,2,60),depth,height:number(value?.height,4,2,15),reach:number(value?.reach,depth,.5,depth)};
}
export function roomLayout(room){
  return {width:room.width,depth:room.depth,height:room.height,positions:{},environmentBrightness:room.environmentBrightness,style:room.style,room:true,lightMin:room.depth-room.reach};
}
...
export function createRoomSettings(host,{getLayout,onChange}){
  ...
  q('brightness').oninput=()=>commit({environmentBrightness:Number(q('brightness').value)});
  q('style').onchange=()=>commit({style:q('style').value});
