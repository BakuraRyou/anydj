...
export function createZonePlan(host,{getLayout,onChange,onPosition,getAims,onAim,getSettings=null,onSave=null}){
  ...
  function commit(){if(getAims)settings.aims=getAims();settings=zoneSettings(settings);if(onSave){onSave(settings);draw();onChange();return;}try{localStorage.setItem(key,JSON.stringify(settings));q('status').textContent='Zonenplan gespeichert.';}catch{q('status').textContent='Zonenplan gilt für diese Sitzung.';}draw();onChange();}
  ...
  q('select').onchange=()=>{selected=q('select').value;draw();if(getSettings)onChange();};q('fixture').onchange=draw;
