...
try {
 ...
 await evaluate(`for(let i=0;i<8;i++)document.querySelectorAll('#trackList [aria-label="In Warteschlange einreihen"]')[i%2].click();document.querySelector('#fadeDuration').value='2';document.querySelector('#autoBeat').checked=false;document.querySelector('#queueStart').click();
 ...
 for(let i=0;i<7;i++){
  ...
  await wait(`document.querySelectorAll('.dj-track-title')[${to}].textContent.endsWith(${JSON.stringify(to?'.mp3':'.wav')})&&document.querySelectorAll('.dj-deck audio')[${to}].readyState>=1`);
  await new Promise(r=>setTimeout(r,250));
  await evaluate(`{const outgoing=document.querySelectorAll('.dj-deck audio')[${from}],incoming=document.querySelectorAll('.dj-deck audio')[${to}];${i===2?"incoming.dataset.delayNext='true';outgoing.currentTime=outgoing.duration-.2;":"outgoing.currentTime=9.8;"}}`);
 ...
 console.log('Eight-track queue passed: seven alternating transitions, delayed incoming playback across outgoing track end, no lost entries, clean finish.');
