...
try {
  ...
  await openRoom();
  assert.equal(await evaluate("document.querySelector('[data-ar-room]').disabled"),true);
  ...
  const initial=await stored(),original=initial.plans[0];
  ...
  let saved=await stored();assert.equal(saved.plans.length,2);const copied=saved.selected;
  ...
  await evaluate("document.querySelector('[data-ar-new-room]').click()");
  saved=await stored();assert.equal(saved.plans.length,3);assert.equal(Object.keys(saved.plans.find(p=>p.id===saved.selected).positions).length,0);
  ...
  console.log('Room library passed: create, duplicate devices/zones, isolated changes, switching, reload and desktop/mobile layout.');
