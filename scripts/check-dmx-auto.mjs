...
try {
  ...
  await evaluate(`window.editStage=(key,value)=>{const p=[...document.querySelectorAll('.dmx-stage')].at(-1),e=p.querySelector('[data-'+key+']');e.value=value;e.dispatchEvent(new Event(key==='mode'||key==='count'||key==='target'||key==='scope'||key==='group'?'change':'input',{bubbles:true}));};
  ...
  await evaluate("editStage('mode','auto');editStage('count','4');fixtureStage.update(liveSource.frame,true,[{...liveSource,look:'held'}])");
  assert.equal(await evaluate("[...document.querySelectorAll('[data-palette]')].at(-1).children.length"),1);
  assert.equal(await evaluate("new Set(Array.from([...document.querySelectorAll('.stage-spots')].at(-1).children,e=>e.style.getPropertyValue('--stage-color'))).size"),1);
  await evaluate("fixtureStage.update(liveSource.frame,true,[{...liveSource,look:'peak'}])");
  assert.equal(await evaluate("[...document.querySelectorAll('[data-palette]')].at(-1).children.length"),4);
  await evaluate("editStage('count','2')");
  assert.equal(await evaluate("[...document.querySelectorAll('[data-palette]')].at(-1).children.length"),2);
  await evaluate("editStage('mode','shared')");
  assert.equal(await evaluate("[...document.querySelectorAll('.stage-spots')].at(-1).children[0].style.getPropertyValue('--stage-color')"),'rgb(128,0,0)');
  await evaluate("editStage('mode','auto');editStage('count','4')");
  ...
  assert.equal(await evaluate("document.querySelector('[data-count]').value"),'4');
  assert.equal(await evaluate("document.querySelector('[data-editor]').hidden"),true);
  await evaluate("document.querySelector('[data-demo]').click()");
  await writeFile(new URL('../reports/dmx-auto-desktop.png',import.meta.url),Buffer.from((await c('Page.captureScreenshot',{format:'png'})).data,'base64'));
  ...
  await writeFile(new URL('../reports/dmx-auto-mobile.png',import.meta.url),Buffer.from((await c('Page.captureScreenshot',{format:'png'})).data,'base64'));
  ...
  console.log('DMX auto browser checks passed: adaptive maximum, calm monochrome, peak palette, auto reload, independent color, shared-mode restore, sections, persistence, demo, blackout, stop, live frame, mobile bounds, keyboard, no hardware session.');
} finally {
