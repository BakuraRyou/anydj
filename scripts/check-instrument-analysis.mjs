...
for(const file of process.argv.slice(2)){
 ...
 try{
  ...
  const grid=await beat.run(pcm,{...beat,signal:new AbortController().signal});
  const musicStyle=await style.run(pcm,{...style,signal:new AbortController().signal});
  ...
  const raw=await structure.run(work,{...structure,signal:new AbortController().signal});
  ...
  const entry={file:basename(file),duration,baseSeconds,refinementSeconds:(performance.now()-modelStart)/1000,totalSeconds:(performance.now()-started)/1000,
   envelopeFrames:refined.instruments.drums.length,sections:plan.arrangement.passages.map(s=>({start:s.start,end:s.end,label:s.label,look:s.look,intensity:s.intensity})),
 ...
 await writeFile(new URL('../reports/instrument-analysis-benchmark.json',import.meta.url),JSON.stringify(results,null,2)+'\n');
}
