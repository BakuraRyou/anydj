...
try {
 ...
 const crowdPixels=()=>evaluate("document.querySelector('.stage-3d-viewport canvas').toDataURL()");
 await new Promise(r=>setTimeout(r,150));const pausedGuests=await crowdPixels();
 await new Promise(r=>setTimeout(r,200));assert.ok((await crowdPixels())===pausedGuests,'loaded paused song keeps guests still');
 ...
 await new Promise(r=>setTimeout(r,150));const dancingGuests=await crowdPixels();
 await new Promise(r=>setTimeout(r,200));assert.ok((await crowdPixels())!==dancingGuests,'guests animate during audio playback');
 await c('Input.dispatchKeyEvent',{type:'keyDown',key:'k',code:'KeyK'});await wait("deckA.querySelector('audio').paused");
 await new Promise(r=>setTimeout(r,150));const stoppedGuests=await crowdPixels();
 await new Promise(r=>setTimeout(r,200));assert.ok((await crowdPixels())===stoppedGuests,'pause freezes the dance');
 ...
 assert.equal(await evaluate("Boolean(document.querySelector('.stage-3d-device-editor [data-add-spot]'))"),true,'equipment and placement share one manager');
 await evaluate("var countBefore=document.querySelectorAll('[data-layout-fixture]').length;document.querySelector('[data-add-spot]').click()");
 await wait("document.querySelectorAll('[data-layout-fixture]').length===countBefore+1");
 await evaluate("var addedDevice=document.querySelector('[data-layout-device]').value;var position=document.querySelector('[data-layout-x]');position.value=2;position.dispatchEvent(new Event('change'))");
 assert.equal(await evaluate("JSON.parse(localStorage.getItem('anydj-stage-layout-v1')).positions[addedDevice].x"),2,'new device is selected and can be positioned immediately');
 await evaluate("document.querySelector('[data-add-bar]').click()");
 await wait("document.querySelectorAll('[data-layout-fixture]').length===countBefore+2");
 await evaluate("var lastRow=[...document.querySelectorAll('.stage-device-row')].at(-1);var cells=lastRow.querySelector('input[type=number]');cells.value=12;cells.dispatchEvent(new Event('change'));document.querySelector('[data-position-device=\"'+addedDevice+'\"]').click()");
 assert.equal(await evaluate("document.querySelector('[data-layout-device]').value===addedDevice"),true,'device list selects the matching map inspector');
 ...
 await evaluate("[...document.querySelectorAll('.stage-device-row')].at(-1).querySelector('button[aria-label]').click();[...document.querySelectorAll('.stage-device-row')].at(-1).querySelector('button[aria-label]').click()");
 await wait("document.querySelectorAll('[data-layout-fixture]').length===countBefore");
 ...
 await c('Input.dispatchMouseEvent',{type:'mousePressed',...zonePoint,button:'left',clickCount:1});
 await c('Input.dispatchMouseEvent',{type:'mouseMoved',x:zonePoint.x+15,y:zonePoint.y,button:'left',buttons:1});
 await c('Input.dispatchMouseEvent',{type:'mouseReleased',x:zonePoint.x+15,y:zonePoint.y,button:'left',clickCount:1});
 assert.ok(await evaluate("JSON.parse(localStorage.getItem('anydj-3d-zones-v1')).zones[0].x")>zoneBefore,'zone moves in the same device plan');
 var aimHandle=await evaluate("(()=>{const h=document.querySelector('[data-layout-target]'),r=h.getBoundingClientRect();window.fixtureId=h.dataset.layoutTarget;return {x:r.x+r.width/2,y:r.y+r.height/2}})()");
 await c('Input.dispatchMouseEvent',{type:'mousePressed',...aimHandle,button:'left',clickCount:1});
 await c('Input.dispatchMouseEvent',{type:'mouseMoved',x:aimHandle.x+15,y:aimHandle.y-15,button:'left',buttons:1});
 await c('Input.dispatchMouseEvent',{type:'mouseReleased',x:aimHandle.x+15,y:aimHandle.y-15,button:'left',clickCount:1});
 assert.equal(await evaluate("Boolean(JSON.parse(localStorage.getItem('anydj-stage-layout-v1')).targets[fixtureId])"),true,'aim uses the same shared layout storage');
 await evaluate("var checks=[...document.querySelectorAll('[data-select-device]')];checks[1].checked=true;checks[1].dispatchEvent(new Event('change',{bubbles:true}));var group=document.querySelector('[data-layout-group]');group.value='2';group.dispatchEvent(new Event('change'));var gain=document.querySelector('[data-layout-gain]');gain.value=70;gain.dispatchEvent(new Event('change'))");
 assert.equal(await evaluate("JSON.parse(localStorage.getItem('anydj-stage-design-v1')).equipment.devices.slice(0,2).every(d=>d.gain===70&&d.group===2)"),true,'mixed selection shares light and group properties');
 await evaluate("var boxes=[...document.querySelectorAll('[data-select-device]')];boxes[4].checked=true;boxes[4].dispatchEvent(new Event('change',{bubbles:true}));var chosen=[...document.querySelectorAll('[data-select-device]:checked')].map(e=>e.dataset.selectDevice);var beforeMove=chosen.map(id=>document.querySelector('[data-layout-fixture=\"'+id+'\"]').transform.baseVal.getItem(0).matrix.e);var moveInput=document.querySelector('[data-layout-x]');moveInput.value=Number(moveInput.value)+.2;moveInput.dispatchEvent(new Event('change'))");
 assert.equal(await evaluate("chosen.every((id,i)=>Math.abs(document.querySelector('[data-layout-fixture=\"'+id+'\"]').transform.baseVal.getItem(0).matrix.e-beforeMove[i]-.2)<.001)"),true,'mixed device groups move together without changing spacing');
 assert.equal(await evaluate("document.querySelector('[data-layout-motion-label]').hidden"),true,'incompatible type-specific properties stay hidden for mixed selections');
 await evaluate("document.querySelector('[data-add-moving]').click()");
 await wait("document.querySelectorAll('.stage-moving-head').length===5&&document.querySelector('[data-layout-device]').value===JSON.parse(localStorage.getItem('anydj-stage-design-v1')).equipment.devices.at(-1).id");
 await evaluate("var movingId=document.querySelector('[data-layout-device]').value;var deviceName=document.querySelector('[data-layout-name]');deviceName.value='Decke Mitte';deviceName.dispatchEvent(new Event('change'));var range=document.querySelector('[data-layout-motion]');range.value=35;range.dispatchEvent(new Event('change'))");
 assert.equal(await evaluate("JSON.parse(localStorage.getItem('anydj-stage-design-v1')).equipment.devices.find(d=>d.id===movingId).motionRange"),.35,'moving head properties share the inspector');
 await evaluate("document.querySelector('[data-position-device=\"'+movingId+'\"]').closest('.stage-device-row').querySelector('button[aria-label]').click()");
 await wait("document.querySelectorAll('.stage-moving-head').length===4");
 assert.equal(await evaluate("(()=>{const d=document.querySelector('.stage-3d-dialog'),p=document.querySelector('.stage-3d-inspector-body');return d.scrollWidth<=d.clientWidth&&p.scrollWidth<=p.clientWidth})()"),true,'unified manager fits mobile width');
