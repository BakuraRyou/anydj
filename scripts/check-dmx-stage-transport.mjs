...
try {
 ...
 await evaluate("document.querySelector('[data-workspace-tab=room]').click();document.querySelector('[data-zone-add]').click()");
 assert.equal(await evaluate("JSON.parse(localStorage.getItem('anydj-3d-zones-v1')).zones.length"),1,'room zone saved');
 await evaluate("var zoneName=document.querySelector('[data-zone-name]');zoneName.value='Tische';zoneName.dispatchEvent(new Event('change'));var zoneWidth=document.querySelector('[data-zone-width]');zoneWidth.value=2;zoneWidth.dispatchEvent(new Event('change'))");
 assert.equal(await evaluate("JSON.parse(localStorage.getItem('anydj-3d-zones-v1')).zones[0].name"),'Tische');
 await evaluate("document.querySelector('[data-zone-map]').scrollIntoView({block:'center'})");
 var zonePoint=await evaluate("(()=>{const map=document.querySelector('[data-zone-map]'),r=map.querySelector('[data-zone-id] rect'),p=new DOMPoint(+r.getAttribute('x')+12,+r.getAttribute('y')+20).matrixTransform(map.getScreenCTM());return {x:p.x,y:p.y}})()");
 ...
 await c('Input.dispatchMouseEvent',{type:'mouseMoved',x:zonePoint.x+20,y:zonePoint.y,button:'left',buttons:1});
 await c('Input.dispatchMouseEvent',{type:'mouseReleased',x:zonePoint.x+20,y:zonePoint.y,button:'left',clickCount:1});
 assert.ok(await evaluate("JSON.parse(localStorage.getItem('anydj-3d-zones-v1')).zones[0].x")>zoneBefore,'zone can be dragged on the map');
 await evaluate("var fixture=document.querySelector('[data-zone-fixture]');fixture.selectedIndex=1;fixture.dispatchEvent(new Event('change'));var map=document.querySelector('[data-zone-map]'),point=new DOMPoint(220,110).matrixTransform(map.getScreenCTM());map.dispatchEvent(new PointerEvent('pointerdown',{clientX:point.x,clientY:point.y,button:0,bubbles:true}))");
 assert.equal(await evaluate("Object.keys(JSON.parse(localStorage.getItem('anydj-3d-zones-v1')).aims).length"),1,'fixed fixture aim saved');
 assert.equal(await evaluate("document.querySelectorAll('[data-zone-lights] line').length"),1,'aim direction shown on map');
 await c('Emulation.setDeviceMetricsOverride',{width:1440,height:1000,deviceScaleFactor:1,mobile:false});
 await evaluate("document.querySelector('.stage-3d-zone-plan').scrollIntoView({block:'start'})");
 await new Promise(r=>setTimeout(r,120));
 await writeFile(new URL('../reports/dmx-zone-plan.png',import.meta.url),Buffer.from((await c('Page.captureScreenshot',{format:'png'})).data,'base64'));
 await evaluate("document.querySelector('[data-zone-reset]').click()");
 assert.equal(await evaluate("Object.keys(JSON.parse(localStorage.getItem('anydj-3d-zones-v1')).aims).length"),0,'fixture default aim restored');
 await evaluate("fixture.value='';fixture.dispatchEvent(new Event('change'));document.querySelector('[data-zone-delete]').click()");
