...
try {
  ...
  async function viewerWait(expression){const end=Date.now()+15000;while(Date.now()<end){if(await read(expression))return;await new Promise(r=>setTimeout(r,100));}throw Error('Viewer timeout: '+expression+' '+JSON.stringify(errors)+' '+await read("document.body.innerText"));}
  sim=spawn(process.execPath,['scripts/start-vr-sim.mjs','--no-open'],{env:{...process.env,VR_SIM_SOURCE:local.origin,VR_SIM_PORT:'3039'},stdio:['ignore','pipe','inherit']});
  await new Promise((resolve,reject)=>{const timer=setTimeout(()=>reject(Error('Simulator startup timeout')),10000);sim.stdout.on('data',data=>{if(String(data).includes('VR-Simulation:')){clearTimeout(timer);resolve();}});sim.once('exit',()=>{clearTimeout(timer);reject(Error('Simulator exited'));});});
  const simulator='http://127.0.0.1:3039';
  assert.equal((await fetch(simulator+'/api/meta')).status,404);
  assert.equal((await fetch(simulator+'/vr-test',{headers:{Origin:'http://foreign.invalid'}})).status,403);
  await vc('Page.navigate',{url:simulator+'/vr-test'});
  await viewerWait("document.querySelector('#enterVR')?.getAttribute('aria-pressed')==='true'");
  await viewerWait("document.querySelectorAll('canvas').length>=3");
  ...
  assert.equal(await read(`(async()=>{const pair=await (await fetch('/api/vr-preview/test-connect')).json();return (await fetch('/api/vr-preview/command',{method:'POST',headers:{'Content-Type':'application/json','X-AnyDj-Local':'1'},body:JSON.stringify({...pair,command:{action:'seek',deck:'A',value:12}})})).status})()`),200);
  await wait("Math.abs(Number(document.querySelector('.dj-seek').value)-12)<.5");
  ...
  console.log('IWER simulator passed: automatic VR entry, DevUI canvas, live room changes, current deck seek, reconnect, origin and route restrictions.');
  await command('Target.closeTarget',{targetId:viewerTarget});
} finally {
  sim?.kill('SIGTERM');ws?.close();chrome.kill('SIGKILL');app.server.closeAllConnections();
