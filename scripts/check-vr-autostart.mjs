...
let app=await createApp({demo:true,previewPort:0});
...
try {
  ...
  await viewerWait("document.querySelector('#connection').textContent==='Live mit dem Rechner verbunden'");
  assert.equal(await read('location.pathname+location.hash'),'/vr-test');
  // Reload a running publisher: restore sharing without opening either stage view.
  assert.equal(await evaluate("localStorage.getItem('anydj-vr-share-resume')"),'true');
  const before=await (await fetch(base+'/api/vr-preview/test-connect')).json();
  await evaluate("localStorage.setItem('anydj-stage-visible','false')");
  await reload();await wait("document.querySelector('[data-share-links]')?.hidden===false");
  ...
  const after=await (await fetch(base+'/api/vr-preview/test-connect')).json();assert.notEqual(after.id,before.id);
  const snapshot=await evaluate(`new Promise((resolve,reject)=>{const timer=setTimeout(()=>{s.close();reject(Error('No resumed scene'));},5000);const s=new EventSource('/api/vr-preview/stream?id=${after.id}');s.onmessage=e=>{clearTimeout(timer);s.close();resolve(JSON.parse(e.data).scene);};})`);
  assert.equal(snapshot.layout.width,14);assert.ok(snapshot.lights.length>0,'hidden stage still publishes fixture positions');
  // Destroy and recreate the backend at the same origin; the publisher recovers itself.
  const mainPort=app.server.address().port,previewPort=Number(new URL(await evaluate("document.querySelector('.stage-vr-share input').value")).port);
  app.server.closeAllConnections();await new Promise(resolve=>app.server.close(resolve));
  app=await createApp({demo:true,previewPort});app.server.listen(mainPort,'127.0.0.1');await once(app.server,'listening');
  const restartDeadline=Date.now()+15000;let recovered;
  while(Date.now()<restartDeadline){try{const response=await fetch(base+'/api/vr-preview/test-connect');if(response.ok){recovered=await response.json();break;}}catch{}await new Promise(resolve=>setTimeout(resolve,150));}
  assert.ok(recovered,'sharing restarts after backend loss');assert.notEqual(recovered.id,after.id);
  await wait("document.querySelector('[data-share-start]').textContent==='Übertragung beenden'");
  // Close the page and backend together, then launch a fresh app at the stable origin.
  await c('Page.navigate',{url:'about:blank'});await wait("location.href==='about:blank'");
  app.server.closeAllConnections();await new Promise(resolve=>app.server.close(resolve));
  app=await createApp({demo:true,previewPort});app.server.listen(mainPort,'127.0.0.1');await once(app.server,'listening');
  await c('Page.navigate',{url:base+'/dj'});await wait("document.querySelector('[data-share-links]')?.hidden===false");
  assert.equal((await fetch(base+'/api/vr-preview/test-connect')).status,200);
  assert.equal(await evaluate("document.querySelector('.stage-3d-dialog').open"),false);
  // An explicit stop clears the preference and survives a new page load.
  await evaluate("document.querySelector('[data-share-start]').click()");
  await wait("localStorage.getItem('anydj-vr-share-resume')==='false'");
  await reload();await wait("document.querySelector('[data-share-start]')");
  await evaluate('new Promise(resolve=>setTimeout(resolve,1200))');
  assert.equal(await evaluate("document.querySelector('[data-share-links]').hidden"),true);
  assert.equal((await fetch(base+'/api/vr-preview/test-connect')).status,404);
  assert.deepEqual(errors,[]);console.log('VR autostart passed: remembered active session, hidden-stage scene, fresh credentials, backend restart and persisted explicit stop.');
