...
try {
 ...
 assert.deepEqual(await evaluate("[...document.querySelector('#decks').children].filter(e=>e.matches('.dj-deck,.dj-mixer')).map(e=>e.classList.contains('dj-mixer')?'Mixer':e.getAttribute('aria-label'))"),['Deck A','Mixer','Deck B']);
 await evaluate(`window.press=(key,target=document.body,extra={})=>target.dispatchEvent(new KeyboardEvent('keydown',{key,bubbles:true,cancelable:true,...extra}));press(' ');`);
 await wait("!deckA.querySelector('audio').paused");
 await evaluate("press('k')");await wait("deckA.querySelector('audio').paused");
 await evaluate("press(' ',document.body,{repeat:true});press('k',deckA.querySelector('.dj-seek'));press(' ',deckA.querySelector('.dj-play'))");
 assert.equal(await evaluate("deckA.querySelector('audio').paused"),true,'repeats and focused controls do not trigger global transport');
 await evaluate("press('ArrowRight',document.body,{shiftKey:true})");
 assert.ok(await evaluate("deckA.querySelector('audio').currentTime>=1"));
 await evaluate("press('ArrowLeft',document.body,{shiftKey:true});window.masterBefore=+document.querySelector('[data-master]').value;press('ArrowDown')");
 assert.ok(await evaluate("Math.abs(+document.querySelector('[data-master]').value-(masterBefore-.05))<.001"));
 await evaluate("press('ArrowUp');const modal=document.createElement('dialog');document.body.append(modal);modal.showModal();press('k');modal.close();modal.remove()");
 assert.equal(await evaluate("deckA.querySelector('audio').paused"),true,'open dialogs isolate deck shortcuts');
