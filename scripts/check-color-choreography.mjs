...
for(const file of process.argv.slice(2)){
 ...
 try{
  ...
  await writeFile('/tmp/anydj-color-comparison.json',JSON.stringify({duration,plan,old:plan.choreographyBaseFrames,auto:plan.frames,disco:disco.frames,events:disco.colorEvents}));
 ...
 await writeFile(new URL('../reports/color-choreography-benchmark.json',import.meta.url),JSON.stringify(results,null,2)+'\n');
}
