...
try {
  ...
  assert.equal(await evaluate("document.querySelector('.stage-bar').hidden"),true);
  await evaluate(`window.setEquipment=(key,value)=>{const e=document.querySelector('[data-equipment-'+key+']');e.value=value;e.dispatchEvent(new Event('change'));};setEquipment('count','2');`);
  assert.equal(await evaluate("document.querySelectorAll('.stage-spot:not([hidden])').length"),2);
  await evaluate("setEquipment('type','bar');setEquipment('count','3')");
  assert.equal(await evaluate("document.querySelector('.stage-spots').hidden"),true);
  assert.equal(await evaluate("document.querySelectorAll('.stage-cell:not([hidden])').length"),3);
  assert.equal(await evaluate("document.querySelector('[data-target] option[value=f0]').disabled"),true);
  assert.equal(await evaluate("document.querySelector('[data-target] option[value=f4]').disabled"),false);
  await evaluate("setEquipment('type','spots')");
  assert.equal(await evaluate("document.querySelector('[data-equipment-count]').value"),'2');
  await evaluate("setEquipment('type','bar');document.querySelector('[data-close]').click()");
  for(const [width,height] of [[1280,720],[1024,768],[390,844]]){
    ...
    await writeFile(new URL(`../reports/dmx-equipment-${width}.png`,import.meta.url),Buffer.from((await c('Page.captureScreenshot',{format:'png'})).data,'base64'));
  }
  ...
  assert.deepEqual(errors,[]);console.log('Equipment passed: type/count, hidden unused controls, per-type count retained, reload, replacement, settings close keeps demo running, activation restored, layouts, deactivate.');
} finally {
