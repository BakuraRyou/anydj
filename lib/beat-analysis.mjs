...
export function runBeatProcess(pcm,{python,checkpoint,device,signal,script=join(root,'scripts/beat-this.py'),extraArgs=[],env=process.env,label='Beat This!'}) {
  return new Promise((resolve,reject)=>{
    if(signal.aborted){reject(signal.reason);return;}
    const child=spawn(python,[script,'--checkpoint',checkpoint,'--device',device,...extraArgs],{stdio:['pipe','pipe','pipe'],env});
...
export class BeatAnalysis {
  constructor({python=process.env.WIZ_BEAT_PYTHON||join(root,'.venv-beat-this/bin/python'),
    ...
    device=process.env.WIZ_BEAT_DEVICE||'cpu',run=runBeatProcess,ready,timeoutMs=180000}={}) {
  ...
  async handle(req,res) {
    ...
    const timer=setTimeout(()=>controller.abort(new AppError('Beat-Analyse dauerte zu lange. Standard wählen oder erneut versuchen.',504)),this.timeoutMs);
