...
import { setupHTTP } from './setup-http.mjs';
...
export class SetupClient {
  constructor({ interfaces = getInterfaces, fetcher = setupHTTP } = {}) {
  ...
  async request(path, body, address = ADDRESS) {
    ...
    if (address === ADDRESS && !interfaces.some(x => x.network === '192.168.56.0' && x.netmask === '255.255.255.0' && x.address !== ADDRESS)) {
      throw new AppError('Den Rechner zuerst mit dem WLAN AnyDjConfig_… der Lampe verbinden.', 409, 'SETUP_NETWORK');
    }
    let response;
    try {
  ...
  async device() {
    ...
    try {
      let response;
      const primary = this.joined && this.homeIP ? this.homeIP : ADDRESS;
      const fallback = primary === ADDRESS ? this.homeIP : ADDRESS;
      try { response = await this.request('/device', undefined, primary); this.deviceAddress = primary; }
      catch (error) {
        if (!fallback) throw error;
        response = await this.request('/device', undefined, fallback);
        this.deviceAddress = fallback;
      }
