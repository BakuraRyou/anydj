...
export class StructureAnalysis {
  constructor({python=join(root,'.venv-structure/bin/python'),cacheDir=join(root,'data/structure'),run=runStructure,ready,timeoutMs=900000}={}) {
    Object.assign(this,{python,cacheDir,run,timeoutMs});this.active=null;this.cache=new Map();
  ...
  async handle(req,res) {
    ...
    const timer=setTimeout(()=>controller.abort(new AppError('Songstruktur-Analyse hat das Zeitlimit erreicht.',504)),this.timeoutMs);
    ...
    try{
      ...
      let size=0;const hash=createHash('sha256');hash.update('all-in-one-infer-3.1.0/harmonix-all/htdemucs/44100-stereo/v1');
