import {validateRoomPlan} from '../public/dmx-ar-model.js';
...
export function createVRPreviewRelay(){
  ...
  return {
    ...
    command(id,control,command){const s=find(id);if(s.control!==control)throw new AppError('Bitte die Brille erneut mit dem Zahlencode koppeln.',403);
      ...
      if(command?.action==='room-plan'){try{command={action:'room-plan',plan:validateRoomPlan(command.plan)};}catch(error){throw new AppError(error.message,400);}if(JSON.stringify(command).length>256000)throw new AppError('Raumplan zu groß.',413);}
      else if(!command||!['select','playing','seek','rate'].includes(command.action)||!['A','B'].includes(command.deck)||command.action==='playing'&&typeof command.value!=='boolean'||['seek','rate'].includes(command.action)&&(!Number.isFinite(command.value)||Math.abs(command.value)>86400))throw new AppError('Ungültiger VR-Befehl.',400);
      ...
      const item={id:++s.commandSeq,time:Date.now(),action:command.action,...(command.action==='room-plan'?{plan:command.plan}:{deck:command.deck}),...(command.value===undefined?{}:{value:command.value})};s.commands.push(item);return {queued:item.id};},
    publish(id,secret,snapshot,ack=[]){const s=owner(id,secret);
      ...
      if(![snapshot.layout.width,snapshot.layout.depth].every(v=>finite(v)&&v>=.5&&v<=60)||!snapshot.layout.positions||typeof snapshot.layout.positions!=='object'||Object.values(snapshot.layout.positions).some(p=>!p||![p.x,p.y,p.height].every(finite))||!snapshot.origin||![snapshot.origin.x,snapshot.origin.y,snapshot.origin.yaw,snapshot.origin.eyeHeight].every(finite)||snapshot.lights.some(l=>!l||![l.position?.x,l.position?.y,l.position?.height,l.target?.x,l.target?.y,l.power].every(finite)||(l.target.z!==undefined&&!finite(l.target.z))||typeof l.color!=='string'||l.color.length>80)||snapshot.crowd.some(p=>!p||![p.x,p.y].every(finite)))throw new AppError('VR-Szene enthält ungültige Geometrie.',400);
      if(snapshot.roomPlan){try{snapshot={...snapshot,roomPlan:validateRoomPlan(snapshot.roomPlan)};}catch(error){throw new AppError(error.message,400);}}
